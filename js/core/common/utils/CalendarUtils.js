...
function addFstv(sYear, sMonth, sDay, weekDay, lunarYear, lunarMonth, lunarDay, isLeap) {
    ...
    // 年龄
    let now = new Date()
    that.age = now.getFullYear() - sYear
...
/**
 * 阳历转农历
 * @param date {Date} Date.parse("1986/02/15") 或 new Date(1986, 1, 15)
 * @returns {*}
 */
CalendarConverter.solar2lunar = function (date) {
 ...
 * @returns {*}
 */
CalendarConverter.lunar2solar = function (date, isLeapMonth) {
 ...
 * @param date
 */
CalendarConverter.getInfo = function(date) {
    let dt = CalendarConverter.solar2lunar(date)
 ...
 *
 */
global.CalendarConverter = CalendarConverter;
